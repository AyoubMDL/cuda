...
@triton.jit
def block_matmul_1d_grid_kernel(a_ptr, b_ptr, c_ptr,
                                M, N, K,
                                stride_am, stride_ak,
                                stride_bk, stride_bn,
                                stride_cm, stride_cn,
    ...
    # We are trying to compute pid_m and pid_n as if we have double axis
    # and because programs are in row major, we work with num_pid_n to find them
...
def block_matmul_1d_grid(A: torch.Tensor, B: torch.Tensor):
    ...
    block_matmul_1d_grid_kernel[grid](A, B, C,
                                      M, N, K,
...
if __name__ == "__main__":
    ...
    C_triton = block_matmul_1d_grid(A, B)
    C_torch = A @ B
