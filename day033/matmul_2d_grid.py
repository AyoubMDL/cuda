...
def matmul_2d_grid_kernel(
    ...
    n = tl.program_id(axis=1)  # column index
...
def naive_matmul_2d_grid(A: torch.Tensor, B: torch.Tensor):
    ...
    matmul_2d_grid_kernel[grid](A, B, C,
                                M, N, K,
...
if __name__ == "__main__":
    torch.manual_seed(0)
    A = torch.randn(10000, 100, device='cuda')
    B = torch.randn(100, 10000, device='cuda')

    C_triton = naive_matmul_2d_grid(A, B)
    C_torch = A @ B
